...
impl ToolPolicy {
    ...
    ///
    /// Reads `{PREFIX}_TOOL_PROFILE` for default and `{PREFIX}_TOOL_PROFILE_{CHANNEL}` per channel.
    #[must_use]
    pub fn with_env_overrides_prefix(mut self, prefix: &str) -> Self {
        // Global default override
        let default_key = format!("{prefix}_TOOL_PROFILE");
        if let Ok(val) = std::env::var(&default_key)
        ...
        for channel in &channels {
            let env_key = format!("{prefix}_TOOL_PROFILE_{}", channel.to_uppercase());
    ...
    /// Apply environment variable overrides using "BEACON" prefix (backward compat)
    #[must_use]
    pub fn with_env_overrides(self) -> Self {
        self.with_env_overrides_prefix("BEACON")
    }
