...
impl WebFetchTool {
    /// Create a new web fetch tool with the specified timeout and default user-agent
    ///
    /// # Errors
    ///
    /// Returns error if the HTTP client cannot be built
    pub fn new(timeout: Duration) -> Result<Self> {
        Self::with_user_agent(timeout, "agent-core/0.1")
    }

    /// Create a new web fetch tool with a custom user-agent
    ///
    /// # Errors
    ///
    /// Returns error if the HTTP client cannot be built
    pub fn with_user_agent(timeout: Duration, user_agent: &str) -> Result<Self> {
    ...
    pub async fn fetch(&self, url: &str, method: Option<&str>) -> Result<WebResponse> {
        ...
        let parsed = reqwest::Url::parse(url)
            .map_err(|e| AgentError::WebFetch(format!("Invalid URL: {e}")))?;

        // Validate scheme
        let scheme = parsed.scheme();
        if scheme != "http" && scheme != "https" {
            return Err(AgentError::WebFetch(format!(
        ...
        let host = parsed
            .host_str()
            .ok_or_else(|| AgentError::WebFetch("URL has no host".to_string()))?;
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| AgentError::WebFetch(format!("Request failed: {e}")))?;
        ...
        let body = response
            .text()
            .await
            .map_err(|e| AgentError::WebFetch(format!("Failed to read response body: {e}")))?;
    ...
    fn check_ssrf(host: &str, port: u16) -> Result<()> {
        ...
        let addrs = addr_str
            .to_socket_addrs()
            .map_err(|e| AgentError::WebFetch(format!("Failed to resolve hostname: {e}")))?;
