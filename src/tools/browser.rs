...
impl BrowserController {
    ...
    pub async fn launch(&self) -> Result<(), BrowserError> {
        ...
        let config = browser_config
            .build()
            .map_err(|e| BrowserError::Launch(format!("config error: {e}")))?;
    ...
    pub async fn navigate(&self, url: &str) -> Result<PageContent, BrowserError> {
        let guard = self.browser.lock().await;
        let browser = guard.as_ref().ok_or(BrowserError::NotRunning)?;

        let page = browser
            .new_page(url)
            .await
            .map_err(|e| BrowserError::Navigation(format!("new page failed: {e}")))?;
    ...
    pub async fn screenshot(&self, url: Option<&str>) -> Result<Screenshot, BrowserError> {
        let guard = self.browser.lock().await;
        let browser = guard.as_ref().ok_or(BrowserError::NotRunning)?;
        ...
        let page = if let Some(url) = url {
            ...
            let pages = browser
                .pages()
                .await
                .map_err(|e| BrowserError::Cdp(format!("get pages failed: {e}")))?;
            pages
                .into_iter()
                .next()
                .ok_or_else(|| BrowserError::Cdp("no active page".to_string()))?
        };
    ...
    pub async fn execute_js(&self, script: &str) -> Result<serde_json::Value, BrowserError> {
        ...
        let result = page
            .evaluate(script)
            .await
            .map_err(|e| BrowserError::Cdp(format!("JS execution failed: {e}")))?;

        result
            .into_value()
            .map_err(|e| BrowserError::Cdp(format!("JS result parse failed: {e}")))
    }
    ...
    pub async fn click(&self, selector: &str) -> Result<(), BrowserError> {
        ...
        let element = page
            ...
            .await
            .map_err(|e| BrowserError::Cdp(format!("element not found: {e}")))?;

        element
            .click()
            .await
            .map_err(|e| BrowserError::Cdp(format!("click failed: {e}")))?;
    ...
    pub async fn type_text(&self, selector: &str, text: &str) -> Result<(), BrowserError> {
        ...
        element
            .click()
            .await
            .map_err(|e| BrowserError::Cdp(format!("focus failed: {e}")))?;

        element
            .type_str(text)
            .await
            .map_err(|e| BrowserError::Cdp(format!("type failed: {e}")))?;
    ...
    pub async fn query_selector_all(
        ...
        let elements = page
            ...
            .await
            .map_err(|e| BrowserError::Cdp(format!("query failed: {e}")))?;
    ...
    pub async fn wait_for_selector(&self, selector: &str) -> Result<(), BrowserError> {
        let guard = self.browser.lock().await;
        let browser = guard.as_ref().ok_or(BrowserError::NotRunning)?;
        ...
        page.find_element(selector)
            .await
            .map_err(|e| BrowserError::Timeout(format!("wait for selector failed: {e}")))?;

        Ok(())
    }

    /// Get page content helper
    async fn get_page_content(&self, page: &Page) -> Result<PageContent, BrowserError> {
        let url = page
            .url()
            .await
            .map_err(|e| BrowserError::Cdp(format!("get URL failed: {e}")))?
            .unwrap_or_default();
        ...
        let html = page
            .content()
            .await
            .map_err(|e| BrowserError::Cdp(format!("get content failed: {e}")))?;
